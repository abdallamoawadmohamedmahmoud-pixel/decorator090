...
<<<<<<< HEAD
        ...
        let users = JSON.parse(localStorage.getItem('decorator_users') || '[]');
        
        // Check if owner exists, if not add them
        const ownerExists = users.some(u => u.email.toLowerCase() === ownerEmail.toLowerCase());
        ...
        if (!ownerExists) {
            ...
            // Fix existing owner data if needed
            users = users.map(u => {
                if (u.email.toLowerCase() === ownerEmail.toLowerCase()) {
                    return {
                        ...u,
                        email: u.email.toLowerCase(),
                        password: ownerPassword, // Reset to correct password
                        is_owner: true,
                        role: 'owner',
                        phone: ownerPassword
                    };
